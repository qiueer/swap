 ...
 */
function load_data_for_select(id, url, where, display_fd, val_fd, value, extra_display, extra_val,style_css_conf)
{
	var post_data = new Object();
	post_data.extra_display = "";
	post_data.extra_val = "";
	
	post_data.id = id;
	post_data.display_fd = display_fd;
	post_data.val_fd = val_fd;
	if(value){
	...
	for(var fd  in where){
		if(typeof(where[fd]) != "function"){
			post_data[fd] = where[fd];
		}
	}
	
	if(extra_display && extra_val ){
		post_data.extra_display = extra_display;
		post_data.extra_val = extra_val;
	}
